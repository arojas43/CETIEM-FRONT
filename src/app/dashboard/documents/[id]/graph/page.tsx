...
import { Database, RefreshCw, Link, Bug } from "lucide-react";
...
export default function DocumentGraphPage() {
  ...
  useEffect(() => { loadGraphData(); }, [documentId]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="flex items-center justify-between px-8 py-5 border-b border-border">
        ...
        <div className="flex items-center gap-2">
          <button onClick={loadDebugInfo} disabled={loading}
            className="flex items-center gap-2 text-xs border border-border hover:border-economia-warning/40 text-muted-foreground hover:text-foreground px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50">
          ...
          <div className="bg-economia-warning/5 border border-economia-warning/20 rounded-2xl p-5 mb-6">
            ...
            <div className="grid md:grid-cols-5 gap-3 mb-4">
              {[
                ...
                <div key={item.l} className="text-center p-3 bg-muted rounded-xl">
            ...
            {debugInfo.countByDocument.length > 0 && (
              <div className="space-y-1">
                <h4 className="text-xs font-medium text-muted-foreground mb-2">Entidades por Documento:</h4>
                {debugInfo.countByDocument.map((doc, i) => (
                  <div key={i} className="flex justify-between text-xs p-2 bg-muted rounded-lg">
                    <span className="font-mono text-muted-foreground truncate flex-1">{doc.docId}</span>
                    <span className="font-bold text-economia-info ml-4">{doc.count}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="bg-card border border-border rounded-2xl p-5 mb-4">
          ...
          {loading ? (
            <div className="text-center py-12">
              <RefreshCw className="h-7 w-7 mx-auto mb-3 animate-spin text-muted-foreground" />
              <p className="text-muted-foreground text-sm">Cargando grafo de conocimiento...</p>
            </div>
          ) : entities.length === 0 ? (
            <div className="text-center py-12">
              <Database className="h-10 w-10 mx-auto mb-3 text-muted-foreground/20" />
              <p className="font-medium text-foreground mb-1">No se encontraron entidades</p>
              <p className="text-sm text-muted-foreground">El documento puede estar procesándose aún</p>
            </div>
          ) : (
                ...
                <div key={type}>
                  ...
                  <div className="space-y-2">
                    {typeEntities.map(entity => (
                      <div key={entity.id} className="flex items-start gap-3 p-3 bg-muted rounded-xl hover:bg-muted transition-colors">
                        <div className="h-8 w-8 bg-economia-info/10 rounded-lg flex items-center justify-center shrink-0">
          ...
          <ul className="space-y-1 text-sm text-muted-foreground">
            {["Extrae entidades clave (empresas, normas, requisitos)", "Identifica y mapea relaciones entre ellas", "Permite búsqueda semántica avanzada sobre el contenido", "Enriquece el análisis del Assessor ESG"].map(item => (
              <li key={item} className="flex items-center gap-2">
                <span className="h-1.5 w-1.5 bg-economia-info rounded-full shrink-0" />{item}
              </li>
            ))}
          </ul>
        </div>
